...
public class ItemDCMetroPSD extends ItemExtension implements IBlock {
    ...
    public ItemDCMetroPSD(EnumPSDAPGItem item, EnumPSDAPGType type, ItemSettings itemSettings) {
        super(itemSettings);
        this.item = item;
        this.type = type;
    }
    ...
    private BlockState getBlockStateFromItem() {
        switch (item) {
            case DOOR:
                return ModBlocks.DC_METRO_PSD_DOOR.get().getDefaultState();
            case GLASS:
                return ModBlocks.DC_METRO_PSD_GLASS.get().getDefaultState();
            default:
