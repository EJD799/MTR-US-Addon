...
public class ItemDCMetroPSD2 extends ItemExtension implements IBlock {
    private final EnumPSDAPGItem item;
    private final EnumPSDAPGType type;

    public ItemDCMetroPSD2(EnumPSDAPGItem item, EnumPSDAPGType type, ItemSettings itemSettings) {
        super(itemSettings);
        this.item = item;
        this.type = type;
    }
    ...
    private BlockState getBlockStateFromItem() {
        switch (item) {
            case DOOR:
                return ModBlocks.DC_METRO_PSD_2_DOOR.get().getDefaultState();
            case GLASS:
                return ModBlocks.DC_METRO_PSD_2_GLASS.get().getDefaultState();
            default:
