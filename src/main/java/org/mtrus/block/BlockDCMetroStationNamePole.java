...
public class BlockDCMetroStationNamePole extends BlockExtension implements IBlock, DirectionHelper, BlockWithEntity {

    public BlockDCMetroStationNamePole(BlockSettings blockSettings) {
        super(blockSettings);
    }
    ...
    public BlockEntityExtension createBlockEntity(BlockPos pos, BlockState state) {
        return new BlockDCMetroStationNamePoleEntity(pos, state);
    }
    ...
    public void onPlaced2(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        if (!world.isClient()) {
            ...
            BlockState endState = ModBlocks.DC_METRO_STATION_NAME_POLE_END.get()
                    .getDefaultState()
    ...
    public void onBreak2(
        ...
        if (world.getBlockState(left).getBlock().data instanceof BlockDCMetroStationNamePoleEnd) {
            IBlock.onBreakCreative(world, player, left);
        }


        if (world.getBlockState(right).getBlock().data instanceof BlockDCMetroStationNamePoleEnd) {
