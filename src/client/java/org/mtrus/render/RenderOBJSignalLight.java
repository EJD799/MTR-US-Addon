...
public class RenderOBJSignalLight extends BlockEntityRenderer<OBJSignalLightEntity> implements IBlock, IGui {

	@SuppressWarnings("deprecation")
	public RenderOBJSignalLight(Argument dispatcher) {
		super(dispatcher);
	}

	@Override
	public final void render(OBJSignalLightEntity entity, float tickDelta, GraphicsHolder graphicsHolder, int light, int overlay) {
		...
		if (!(state.getBlock().data instanceof OBJSignalLight block)) {
			return;
		}
        ...
        MainRenderer.scheduleRender(
                ...
                (graphicsHolderNew, offset) -> {
                    ...
                    Direction facing = IBlock.getStatePropertySafe(state, OBJSignalLight.FACING);
		...
		final float angle = OBJSignalLight.getAngle(state);
		...
		for (int i = 0; i < (entity.isDoubleSided ? 2 : 1); i++) {
			...
			if (aspectState != null) {
				...
				final int aspects = block.getAspects();
				...
				final OptimizedModelWrapper lightModel = lightModelTemp;

				if (lightModel == null) {
					return;
				}

				final StoredMatrixTransformations lightTransform =
						new StoredMatrixTransformations(
								pos.getX() + 0.5,
								pos.getY(),
								pos.getZ() + 0.5
				...
				MainRenderer.scheduleRender(
						...
						(graphicsHolderNew, offset) -> {
							...
							lightTransform.transform(graphicsHolderNew, offset);
