...
public class RenderDCMetroPSD2Door<T extends BlockDCMetroPSD2DoorEntity> extends BlockEntityRenderer<T> implements IGui, IBlock {
    ...
    public RenderDCMetroPSD2Door(Argument dispatcher, int type) {
        super(dispatcher);
        this.type = type;
    }
    ...
    public void render(T entity, float tickDelta, GraphicsHolder graphicsHolder, int light, int overlay) {
                ...
                if (half) {
                    ...
                    if (block.data instanceof BlockAPGGlass || block.data instanceof BlockAPGGlassEnd) {
                        MainRenderer.scheduleRender(new Identifier(String.format("mtrus:textures/block/dc_metro_psd_2_door_light_%s.png", open > 0 ? "on" : "off")), false, QueuedRenderLayer.LIGHT_TRANSLUCENT, (graphicsHolderNew, offset) -> {
                            ...
                            graphicsHolderNew.translate(side ? -0.484375 : 0.515625, 2.3, 0.1);
                            graphicsHolderNew.scale(0.5F, 3.5F, 1);
                            MODEL_APG_LIGHT.render(graphicsHolderNew, light, overlay, 1, 1, 1, 1);
                            graphicsHolderNew.pop();
                        });

                        MainRenderer.scheduleRender(new Identifier(String.format("mtrus:textures/block/psd_2_side_light.png")), false, QueuedRenderLayer.EXTERIOR, (graphicsHolderNew, offset) -> {
                            storedMatrixTransformationsLight.transform(graphicsHolderNew, offset);
        ...
        switch (type) {
            case 2:
                MainRenderer.scheduleRender(new Identifier(String.format("mtrus:textures/block/dc_metro_psd_2_door_%s_%s.png", half ? "top" : "bottom", side ? "right" : "left")), false, QueuedRenderLayer.EXTERIOR, (graphicsHolderNew, offset) -> {
