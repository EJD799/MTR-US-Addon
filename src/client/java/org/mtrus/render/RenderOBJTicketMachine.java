...
public class RenderOBJTicketMachine extends BlockEntityRenderer<OBJTicketMachineEntity> {

    @SuppressWarnings("deprecation")
    public RenderOBJTicketMachine(BlockEntityRenderer.Argument dispatcher) {
        super(dispatcher);
    }

    @Override
    public void render(
            OBJTicketMachineEntity entity,
            float tickDelta,
        ...
        if (IBlock.getStatePropertySafe(state, OBJTicketMachine.HALF) == DoubleBlockHalf.LOWER) {

            final Object blockObject = state.getBlock().data;

            if (!(blockObject instanceof OBJTicketMachine block)) {
                return;
            }
    ...
    @Override
    public boolean rendersOutsideBoundingBox2(OBJTicketMachineEntity entity) {
        return true;
    }
